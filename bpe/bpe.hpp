...
void readText(const char *fp, unordered_map<string, uint32_t> &word_count) {
    ...
    if (string(fp).compare("-") == 0) { // from std input
    ...
    fprintf(stderr, "Read %llu words (%lu unique) from text file.\n", total,
                    word_count.size());
}
...
int outputText(const char *fpo, const char *fp,
...
int readVocab(const char *fp, unordered_map<string, uint32_t> &vocab) {
    int ret = 0;
    ifstream file(fp);
    if (!file) {
    ...
    while (getline(file, line)) {
        ...
        split(splits, line, ' ');
        if (splits.size() != 2) {
            fprintf(stderr, "Loading vocabulary (line %llu) error ... ", lc);
            ret = 1;
            break;
        }
        if (vocab.find(splits[0]) == vocab.end()) {
            fprintf(stderr, "Loading vocabulary (vocab already find (%s) error ... ", splits[0].c_str());
            ret = 2;
            break;
        }
    ...
    if (!ret) {
        fprintf(stderr, "Read %llu words (%lu unique) from vocabulary file.\n", total, vocab.size());
    }
...
int readCodes(const char *fp, unordered_map<tps, uint32_t, pair_hash> &codes,
    ...
    while (getline(file, line)) {
        ...
        // support version 0.2
        if (splits.size() != 2) {
            fprintf(stderr, "Loading codes (line %llu) error ... ", lc);
            ret = 1;
            break;
        }
        ...
        if (codes.find(pair) != codes.end()) {
            fprintf(stderr, "Loading codes (codes already find (%s, %s) error ... ", pair.first.c_str(),
                    pair.second.c_str());
            ret = 2;
            break;
        }
        if (reversed_codes.find(concat) != reversed_codes.end()) {
            fprintf(stderr, "Loading codes (reversed_codes already find %s) error ... ", concat.c_str());
            ret = 3;
            break;
        }
...
public:
    ...
    int init(const string& codesPath, const string& vocabPath) {
        int ret = 0;
        do {
            if (vocabPath.size() > 0) {
                ret = readVocab(vocabPath.c_str(), vocab);
                if (ret) { break; }
            }
            ret = readCodes(codesPath.c_str(), codes, reversed_codes);
        } while(0);
...
void applybpe_stream(const char *codesPath, const char *vocabPath) {
    BPEApplyer applyer;
    int ret = applyer.init(codesPath, vocabPath);
    if (ret) {
        fprintf(stderr, "BPEApplyer init fail!");
        return ;
    }
    std::string line;
